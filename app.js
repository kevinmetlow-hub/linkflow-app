...
function renderServicesList(){
  ...
  if(state.services===null){
    box.innerHTML = '<div class="service-card skeleton-card"><div class="skeleton-line w55"></div><div class="skeleton-line w80"></div></div>';
    return;
  }
...
async function submitPublicBooking(){
  ...
  if(!svc){ setInlineStatus("customerSubmitError","Choose a service first.","error"); return; }
  ...
  if(!customerName || !phone || !address){
    setInlineStatus("customerSubmitError","Enter your name, phone, and address.","error");
    return;
  }
